...
#-------------------------------------------------------------------------------
# Code to be run in preparation for Figure 4
# Uses caveclient to find input to the inputs of NSCs: unique_interneurons_v783.csv
# File generated at beginning of Figure_4.R
#-------------------------------------------------------------------------------
...
sensory_int = pd.read_csv(("./input/unique_interneurons_v"+v+".csv"), sep=",") # edit

sensory_int_id = sensory_int["pre_pt_root_id"]
syn_df = client.materialize.query_table("synapses_nt_v1", 
                                  filter_in_dict={"post_pt_root_id" : sensory_int_id},
...
outpath = "./input/tmp/interneuron_input_filtered_v"+v+".csv"
result.to_csv(outpath)

print(f"Saved filtered interneuron input data to: {os.path.abspath(outpath)}")
